...
const currentUser = {
  ...
  phone: "089-111-1111",
  address: "อาคาร A ชั้น 12 กทม."
};

const initialControllerData: RopaData[] = [
  { 
    id: "C001", record_type: "Controller", request_type: "สร้างรายการใหม่", activity_name: "ระบบจัดการเงินเดือนพนักงาน", purpose: "คำนวณและจ่ายเงินเดือน", data_subject_category: "พนักงาน", collected_personal_data: "ชื่อ, เบอร์, เลขบัญชี", legal_basis: "Contract", status: "Approved",
    created_at: "10/4/2569 10:30:00", created_by: "System Admin", updated_at: "12/4/2569 14:20:00", updated_by: "System Admin", approved_by: "Karnkitti (DPO)",
    controller_info: "บจก. ตัวอย่าง", data_type: "ข้อมูลทั่วไป", collection_format: "Soft File", is_direct_from_subject: "true"
  },
  { 
    id: "C002", record_type: "Controller", request_type: "สร้างรายการใหม่", activity_name: "ระบบวิเคราะห์พฤติกรรมลูกค้า", purpose: "ปรับปรุงและพัฒนาสินค้า", data_subject_category: "ลูกค้า", collected_personal_data: "ประวัติการซื้อ, คุกกี้", legal_basis: "Legitimate Interest", status: "Rejected",
    created_at: "11/4/2569 09:00:00", created_by: "System Admin", updated_at: "11/4/2569 09:00:00", updated_by: "System Admin", approved_by: "-",
    controller_info: "บจก. ตัวอย่าง", data_type: "ข้อมูลทั่วไป", collection_format: "Soft File", is_direct_from_subject: "true",
    rejection_reason: "กรุณาระบุมาตรการเชิงเทคนิคในข้อ 15.2 ให้ชัดเจนยิ่งขึ้น และตรวจสอบฐานการประมวลผลอีกครั้ง"
  }
];

const initialProcessorData: RopaData[] = [
  { 
    id: "P001", record_type: "Processor", request_type: "สร้างรายการใหม่", activity_name: "บริการ Cloud Hosting", purpose: "จัดเก็บข้อมูลระบบ HR", data_subject_category: "พนักงาน", collected_personal_data: "ไฟล์ฐานข้อมูล", legal_basis: "Contract", status: "Approved",
    created_at: "20/3/2569 09:15:00", created_by: "System Admin", updated_at: "1/4/2569 11:00:00", updated_by: "System Admin", approved_by: "Karnkitti (DPO)",
    processor_name: "บจก. คลาวด์ โปร", controller_address: "123 ถ.สุขุมวิท กทม.", data_type: "ข้อมูลทั่วไป", collection_format: "Soft File", is_direct_from_controller: "false", indirect_source_detail: "รับมอบจาก Data Controller"
  },
];
...
export default function RoPARecordPage() {
  ...
  // Page States
  const [currentMenu, setCurrentMenu] = useState("Processor");
  ...
  // Data States
  const [controllerData, setControllerData] = useState<RopaData[]>(initialControllerData);
  const [processorData, setProcessorData] = useState<RopaData[]>(initialProcessorData);
  ...
  // Active Data Logic
  const getActiveData = () => {
    const sourceData = currentMenu === "Controller" ? controllerData : processorData;
    if (currentTab === "Activities") {
      return sourceData.filter(item => item.status && ["Approved", "Rejected"].includes(item.status));
    } else {
      return sourceData.filter(item => item.status === "Pending");
    }
  };
  ...
  const filteredData = activeData.filter(item => {
    const matchId = (item.id || "").toLowerCase().includes(filters.id.toLowerCase());
  ...
  const handleSaveSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const timestamp = new Date().toLocaleString('th-TH', { dateStyle: 'short', timeStyle: 'medium' });

    const newRecord: RopaData = {
      ...formData,
      updated_at: timestamp,
      updated_by: currentUser.name,
      status: "Pending", // ทุกการบันทึกจะเป็นการขออนุมัติ
      data_subject: formData.data_subject_category,
      rejection_reason: "" 
    };

    if (modalMode === 'edit' && selectedRecord?.status === 'Approved') {
      newRecord.request_type = "แก้ไข";
    }

    if (modalMode === 'add') {
      newRecord.request_type = "สร้างรายการใหม่";
      const prefix = currentMenu === "Controller" ? "C" : "P";
      const typeRecords = currentMenu === "Controller" ? controllerData : processorData;
      newRecord.id = `${prefix}${String(typeRecords.length + 1).padStart(3, '0')}`;
      newRecord.created_at = timestamp;
      newRecord.created_by = currentUser.name;
      newRecord.approved_by = "-";

      if (currentMenu === "Controller") setControllerData([newRecord, ...controllerData]);
      else setProcessorData([newRecord, ...processorData]);
    } else {
      if (currentMenu === "Controller") setControllerData(controllerData.map(item => item.id === selectedRecord?.id ? { ...item, ...newRecord } : item));
      else setProcessorData(processorData.map(item => item.id === selectedRecord?.id ? { ...item, ...newRecord } : item));
    }
  ...
  const executeDelete = () => {
    ...
    if (recordToDelete.status === 'Approved') {
        // หาก Approved อยู่ ให้ส่งคำขอลบไปที่ DPO (สถานะเป็น Pending)
        const timestamp = new Date().toLocaleString('th-TH', { dateStyle: 'short', timeStyle: 'medium' });
        const updatedRecord = { 
            ...recordToDelete, 
            status: "Pending", 
            request_type: "ลบรายการ", 
            updated_at: timestamp, 
            updated_by: currentUser.name 
        };
        if (currentMenu === "Controller") {
            setControllerData(controllerData.map(item => item.id === recordToDelete.id ? updatedRecord : item));
        } else {
            setProcessorData(processorData.map(item => item.id === recordToDelete.id ? updatedRecord : item));
        }
    } else {
        // หากเป็น Rejected หรือ Pending ให้ลบทิ้งออกจากระบบได้เลย
        if (currentMenu === "Controller") {
            setControllerData(controllerData.filter(item => item.id !== recordToDelete.id));
        } else {
            setProcessorData(processorData.filter(item => item.id !== recordToDelete.id));
        }
    }
    ...
    <div className="h-screen flex flex-col font-sans overflow-hidden bg-slate-50">
                ...
                <div className="bg-slate-50 rounded-lg overflow-hidden border border-slate-100">
                  ...
                  {isRopaMenuOpen && (
                    <div className="flex flex-col pb-1">
                      <button aria-label="ดูข้อมูล Controller" title="Controller Menu" onClick={() => { setCurrentMenu("Controller"); setCurrentTab("Activities"); setCurrentPage(1); handleResetFilters(); }} className={`w-full text-left pl-10 pr-3 py-2 text-xs font-medium transition-colors ${currentMenu === "Controller" ? "text-blue-700 bg-blue-100/50" : "text-slate-600 hover:text-slate-800 hover:bg-slate-100"}`}>Controller</button>
                      <button aria-label="ดูข้อมูล Processor" title="Processor Menu" onClick={() => { setCurrentMenu("Processor"); setCurrentTab("Activities"); setCurrentPage(1); handleResetFilters(); }} className={`w-full text-left pl-10 pr-3 py-2 text-xs font-medium transition-colors ${currentMenu === "Processor" ? "text-blue-700 bg-blue-100/50" : "text-slate-600 hover:text-slate-800 hover:bg-slate-100"}`}>Processor</button>
                    </div>
                  )}
                </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-[280px] p-5 animate-in fade-in zoom-in-95 duration-200">
